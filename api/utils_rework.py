...
# Matching prospects with a freelancer's search
def calculate_similarity_prospects(freelancer_row, prospect_df, vectorizer=None):
    """
...
def get_top_20_prospects(freelancer_row, prospect_df, vectorizer=None):
...
# Matching freelancers with a prospecter's search

def calculate_holistic_similarity(prospecter_row, freelance_df, vectorizer=None):
    """
    Calculates a holistic similarity score between a prospecter and all freelancers,
    combining text, tone, remote preference, sector, and geographical proximity.
    """
    ...
    # Initialize a mask for prospects that meet the distance criteria
    distance_mask = np.ones(len(freelance_df), dtype=bool)

    # Calculate geographical similarity and apply distance filter
    geo_similarities = []
    prospect_city = cleaning(prospecter_row['city']).lower()
    prospecter_coord_row = city_coordinates_df[city_coordinates_df["city"].str.lower() == prospect_city]

    prospecter_lat, prospecter_lon = None, None
    if not prospecter_coord_row.empty:
        prospecter_lat = prospecter_coord_row["lat"].values[0]
        prospecter_lon = prospecter_coord_row["lon"].values[0]

    for i, freelance_row in freelance_df.iterrows():
        # If either freelancer or prospect is remote, geographical proximity is not a constraint (distance 1.0)
        if prospecter_row['remote_flag'] == 1 or freelance_row['remote_flag'] == 1:
            geo_similarities.append(1.0)
        else:
            # Both are not remote, so geographical proximity matters
            freelance_city = cleaning(freelance_row['city']).lower()
            freelance_coord_row = city_coordinates_df[city_coordinates_df["city"].str.lower() == freelance_city]

            if not freelance_coord_row.empty and prospecter_lat is not None:
                freelance_lat = freelance_coord_row["lat"].values[0]
                freelance_lon = freelance_coord_row["lon"].values[0]

                dist = haversine_distance(prospecter_lat, prospecter_lon, freelance_lat, freelance_lon)
                if dist <= 40:
                    geo_similarities.append(1.0)
                else:
                    geo_similarities.append(0.0)
                    distance_mask[i] = False  # Mark this freelance for exclusion
            else:
                geo_similarities.append(0.0)
                distance_mask[i] = False # Mark for exclusion if coordinates are missing
    ...
    # Filter prospect_df based on distance_mask BEFORE other calculations
    filtered_freelance_df = freelance_df[distance_mask].copy()

    if filtered_freelance_df.empty:
        return np.array([]) # No prospects left after distance filtering

    # Recalculate other similarities for the filtered prospects
    # 1. Text Similarity
    if vectorizer:
        prospecter_text_vec = vectorizer.transform([prospecter_row['combined_text']])
        freelance_text_vectors = vectorizer.transform(filtered_freelance_df['combined_text'])
        text_similarities = cosine_similarity(prospecter_text_vec, freelance_text_vectors).flatten()
    else:
        text_similarities = np.zeros(len(filtered_freelance_df)) # Placeholder

    # 2. Tone Similarity
    tone_similarities = np.array([
        1 if prospecter_row['target_tone'].lower() == freelance_tone.lower() else 0
        for freelance_tone in filtered_freelance_df['preferred_tone']
    ])

    # 3. Remote Preference Similarity
    remote_similarities = np.array([
        1 if prospecter_row['remote_flag'] == freelance_remote_flag else 0
        for freelance_remote_flag in filtered_freelance_df['remote_flag']
    ])

    # # 4. Sector Similarity
    sector_similarities = np.array([
        1 if cleaning(prospecter_row['sector']) == cleaning(freelance_sector) else 0
        for freelance_sector in filtered_freelance_df['main_sector']
    ])

    # Apply the geo_similarities calculated *before* filtering to the filtered_freelance_df
    # We need to ensure that geo_similarities aligns with filtered_freelance_df.
    # The simplest way is to re-extract the relevant geo_similarities for the filtered rows.
    # Since `distance_mask` was used to filter `filter_df`, the `geo_similarities` array
    # already has zeros for the excluded rows. We just need to take the elements corresponding
    # to the `True` values in `distance_mask`.
    filtered_geo_similarities = geo_similarities[distance_mask]
    ...
    return final_similarities, filtered_freelance_df

def get_top_20_freelances(prospecter_row, freelance_df, vectorizer=None):

    city_coordinates_df = pd.DataFrame.from_dict(city_coord, orient='index').reset_index()
    city_coordinates_df = city_coordinates_df.rename(columns={'index': 'city'})

    final_similarities, filtered_freelance_df = calculate_holistic_similarity(prospecter_row, freelance_df, vectorizer)

    if filtered_freelance_df.empty:
        print("No freelances found within the 40km radius or fulfilling remote criteria.")
        return pd.DataFrame() # Return an empty DataFrame if no prospects meet the criteria

    # Ensure we don't try to get more leads than available after filtering
    num_leads = min(20, len(filtered_freelance_df))
    top_leads_idx = final_similarities.argsort()[-num_leads:][::-1]
    return filtered_freelance_df.iloc[top_leads_idx].assign(similarity=final_similarities[top_leads_idx])
